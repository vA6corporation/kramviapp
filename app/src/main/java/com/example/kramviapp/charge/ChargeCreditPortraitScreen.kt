...
fun ChargeCreditPortraitScreen(
    ...
    val dues = listOf(CreateDueModel(charge, dueDate.toString()))
    ...
    clickMenu?.let {
        ...
        if (it == "add_init_payment") {
            showInitPaymentsDialog = true
        }
    }
    ...
    Column {
        ...
        Column(
            ...
            if (payments.isNotEmpty()) {
                //Text(text = "Monto a cuenta: ${payments[0].charge}")
                Text(text = "Monto a cuenta: ${String.format("%.2f", payments[0].charge)}")
                Spacer(modifier = Modifier.height(12.dp))
            }
            ...
            ExposedDropdownMenuBox(
                ...
                ExposedDropdownMenu(
                    ...
                    DropdownMenuItem(
                        onClick = {
                            invoiceCode = InvoiceCode.BOLETA
                            expandedInvoice = false
                        },
                        text = {
                            Text(InvoiceCode.BOLETA.toString())
                        }
                    )
                    DropdownMenuItem(
                        onClick = {
                            invoiceCode = InvoiceCode.FACTURA
                            expandedInvoice = false
                        },
                        text = {
                            Text(InvoiceCode.FACTURA.toString())
                        }
                    )
                    DropdownMenuItem(
                        onClick = {
                            invoiceCode = InvoiceCode.NOTA_DE_VENTA
                            expandedInvoice = false
                        },
                        text = {
                            Text(InvoiceCode.NOTA_DE_VENTA.toString())
                        }
                    )
                }
            }
...
//            }
            ...
            if (setting.isShowTotalDiscount) {
                TextField(
                    value = discount,
            ...
            Button(
                ...
                onClick = {
                    turn.let { turn ->
                        ...
                        if (invoiceCode == InvoiceCode.FACTURA) {
                            customer.let {
                                if (it == null) {
                                    navigationViewModel.showMessage("Agrega un cliente")
                                    return@Button
