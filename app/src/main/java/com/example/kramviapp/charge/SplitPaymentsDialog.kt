...
fun SplitPaymentsDialog(
    ...
    LaunchedEffect(Unit) {
        if (payments.isNotEmpty()) {
            ...
            copyList.add(CreatePaymentModel(charge = 0.0, paymentMethodId = paymentMethods[0].id, turnId))
            copyList.add(CreatePaymentModel(charge = 0.0, paymentMethodId = paymentMethods[0].id, turnId))
                ...
                for (payment in localPayments) {
                    ...
                    Row {
                        ...
                        ExposedDropdownMenuBox(
                            ...
                            paymentMethods.find { it.id == payment.paymentMethodId }?.let {
                                TextField(
                                    readOnly = true,
                                    value = it.name,
                                    onValueChange = { },
                    ...
                    Button(
                        onClick = {
                            if (localPayments.find { it.charge <= 0 } == null) {
                                ...
                                val truncatedCharge: Double = BigDecimal.valueOf(charge)
                                    .setScale(2, RoundingMode.HALF_UP)
                                    .toDouble()

                                val truncatedSum: Double = BigDecimal.valueOf(sum)
                                    .setScale(2, RoundingMode.HALF_UP)
                                    .toDouble()

                                if (truncatedCharge.equals(truncatedSum)) {
