...
fun ChangeBoardsScreen(
    ...
    LaunchedEffect(Unit) {
        navigationViewModel.setTitle("Seleccione una mesa")
    ...
    LazyVerticalGrid(
        ...
        tables?.let { tables ->
            items(tables) {table ->
                val board = boards.find { it.tableId == table.id }
                Box(
                    modifier = Modifier
                        .height(100.dp)
                        .padding(.3.dp)
                        ...
                        .clickable {
                            if (board == null) {
                               ...
                               navigationViewModel.showMessage("Esta mesa esta ocupada")
                            }
                        },
                ) {
                    Row(
