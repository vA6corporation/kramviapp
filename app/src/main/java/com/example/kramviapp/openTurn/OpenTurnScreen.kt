...
fun OpenTurnScreen(
    ...
    clickMenu?.let {
        ...
        if (it == "add_observation") {
            showAddobservationDialog = true
        }
        if (it == "print_box") {
            turn?.let { turn ->
                navigationViewModel.loadBarStart()
                openTurnViewModel.loadSummarySaleItemsByTurn(turn.id, onResponse = { summarySaleItems ->
    ...
    LaunchedEffect(Unit) {
        ...
        actions.add(ActionModel("add_observation", "Agregar observaciones", Icons.Default.Info))
    ...
    if (showAddobservationDialog) {
        AddobservationDialog(onDismissRequest = { observation ->
    ...
    Box(modifier = Modifier
        ...
        turn.let { turn ->
            if (turn == null) {
                ...
                LaunchedEffect(Unit) {
                    navigationViewModel.loadBarStart()
                    openTurnViewModel.loadSummaryPaymentsByTurn(turn.id)
                    openTurnViewModel.loadExpensesByTurn(turn.id)
                    navigationViewModel.loadBarFinish()
                }
                ...
                if (showCreateExpenseDialog) {
                    CreateExpenseDialog(turn.id) { expense ->
                ...
                Column(Modifier.verticalScroll(rememberScrollState())) {
                    ElevatedCard(
                        ...
                        Column(modifier = Modifier.padding(12.dp)) {
                            ...
                            Row(
                                ...
                                if (turn.observation.isEmpty()) {
                                    Text(text = "NINGUNO", color = Color.Gray)
                                } else {
                                    Text(text = turn.observation)
                                }
                            }
                    ...
                    Button(
                        onClick = {
                            navigationViewModel.loadBarStart()
                            openTurnViewModel.loadSummarySaleItemsByTurn(turn.id, onResponse = {
                                summarySaleItems = it
                                navigationViewModel.loadBarFinish()
                            })
                        },
