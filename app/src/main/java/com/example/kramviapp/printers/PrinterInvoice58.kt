...
class PrinterInvoice58(
    ...
    private fun buildBody(printer: EscPosPrinter): StringBuilder {
        ...
        body.append("[C]${business.name.uppercase()}\n")
        ...
        for (saleItem in saleItems) {
            ...
            body.append("[L]").append("x").append(saleItem.quantity).append("[R]")
                .append(String.format("%.2f", saleItem.price * saleItem.quantity))
                .append("\n")
        }
        ...
        if (sale.invoiceCode != InvoiceCode.NOTA_DE_VENTA) {
            if (sale.gravado > 0) {
                body.append("[L]OP. GRAVADAS ").append(currency).append("[R]")
                    .append(String.format("%.2f", sale.gravado)).append("\n")
            }
            if (sale.gratuito > 0) {
                body.append("[L]OP. GRATUITAS ").append(currency).append("[R]")
                    .append(String.format("%.2f", sale.gratuito)).append("\n")
            }
            if (sale.exonerado > 0) {
                body.append("[L]OP. EXONERADAS ").append(currency).append("[R]")
                    .append(String.format("%.2f", sale.exonerado)).append("\n")
            }
            if (sale.inafecto > 0) {
                body.append("[L]OP. INAFECTAS ").append(currency).append("[R]")
                    .append(String.format("%.2f", sale.inafecto)).append("\n")
            }
            body.append("[L]IGV ").append(currency).append("[R]")
                .append(String.format("%.2f", sale.igv)).append("\n")
        }

        body.append("[L]TOTAL DCTO ").append(currency).append("[R]").append(sale.discount)
            .append("\n")
        ...
        if (sale.cash != null && sale.cash > 0) {
            body.append("[L]BILLETE ").append(currency).append("[R]")
                .append(String.format("%.2f", sale.cash)).append("\n")
            body.append("[L]VUELTO ").append(currency).append("[R]")
                .append(String.format("%.2f", sale.cash - sale.charge))
                .append("\n")
        }
...
//            }
//            body.append("[C]<qrcode size='15'>").append(qr).append("</qrcode>").append("\n")
//        }

        when (sale.invoiceCode) {
            InvoiceCode.FACTURA -> {
                body.append("[L]")
                    .append("autorizado mediante resolucion N.")
                    .append("0180050001442/SUNAT Representacion")
                    .append("impresa de la FACTURA DE VENTA\n")
                body.append("[L]\n")
            }
            ...
            InvoiceCode.BOLETA -> {
                body.append("[L]")
                    .append("autorizado mediante resolucion N.")
                    .append("0180050001442/SUNAT Representacion")
                    .append("impresa de la BOLETA DE VENTA\n")
                body.append("[L]\n")
            }
        ...
        if (sale.observation.isNotEmpty()) {
            body.append("[C]").append(sale.observation).append("\n")
            body.append("[L]\n")
        }

        if (setting.textBottom.isNotEmpty()) {
            body.append("[C]").append(setting.textBottom).append("\n")
            body.append("[L]\n")
        }
