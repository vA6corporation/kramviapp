...
class PrinterProforma80(
    ...
    private fun buildBody(printer: EscPosPrinter): StringBuilder {
        ...
        for (proformaItem in proformaItems) {
            ...
            body.append("[L]").append(proformaItem.quantity).append("[C]")
                .append(String.format("%.2f", proformaItem.price))
                .append("[R]").append(String.format("%.2f", proformaItem.price * proformaItem.quantity))
                .append("\n")
        }
        ...
        body.append("[L][R]TOTAL DCTO ").append(currency).append("[R]")
            .append(proforma.discount).append("\n")
        body.append("[L][R]IMPORTE TOTAL ").append(currency).append("[R]")
            .append(String.format("%.2f", proforma.charge)).append("\n")

        if (proforma.observation.isNotEmpty()) {
            body.append("[C]").append(proforma.observation).append("\n")
            body.append("[L]\n")
        }
