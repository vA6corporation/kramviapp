...
class PrinterPreaccount58(
    ...
    private fun buildBody(): StringBuilder {
        ...
        body.append("[C]<font size='tall'>PRECUENTA MESA ${table.name}</font>\n")
        body.append("[L]\n")
        for (boardItem in boardItems) {
            body.append("[L]<font size='tall'>")
                .append(String.format("%.0f", boardItem.quantity)).append("  ")
                .append(boardItem.fullName.uppercase())
                .append("[R]").append(String.format("%.2f", boardItem.price * boardItem.quantity))
                .append("</font>\n")
            if (boardItem.observation.isNotEmpty()) {
                body.append("[L]<font size='tall'>- ").append(boardItem.observation).append("</font>\n")
            }
        ...
        body.append("[L]<font size='tall'>").append("IMPORTE TOTAL: ")
            .append(String.format("%.2f", totalCharge)).append("</font>\n")
        body.append("[L]").append("\n")
