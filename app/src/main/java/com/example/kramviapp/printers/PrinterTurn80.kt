...
class PrinterTurn80(
    ...
    private fun buildBody(): StringBuilder {
        ...
        for (expense in expenses) {
            body.append("[L]").append(expense.concept.uppercase()).append("[R]")
                .append(expense.charge).append("\n")
        }

        body.append("[L]\n")

        for (summaryPayment in summaryPayments) {
            if (summaryPayment.paymentMethod.name == "EFECTIVO") {
        ...
        if (turn.observation.isNotEmpty()) {
            body.append("[L]\n")
            body.append("[C]").append(turn.observation).append("\n")
        }
