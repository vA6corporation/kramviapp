        ...
        selectedSale?.let { sale ->
            DeleteInvoiceDialog(
                onSuccessRequest = { deletedReason ->
                    ...
                    if (sale.invoiceCode == InvoiceCode.NOTA_DE_VENTA || business.certificateId == null) {
    ...
    if (showBottomSheet) {
        selectedSale?.let { sale ->
            ...
            InvoicesBottonSheet(
                ...
                onDeleteRequest = {
                    if (business.certificateId != null && sale.cdr == null && sale.invoiceCode != InvoiceCode.NOTA_DE_VENTA) {
                        navigationViewModel.loadBarStart()
                        invoicesViewModel.sendInvoice(sale.id, onResponse = {
                            navigationViewModel.loadBarFinish()
                            sale.cdr = it
